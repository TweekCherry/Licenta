...
public class AppointmentRepositoryCustomImpl implements AppointmentRepositoryCustom {
	...
	public Mono<UpdateResult> cancelByMedic(ObjectId id) {
		...
		update.set("status", AppointmentStatus.CANCELLED);
	...
	public Flux<Appointment> aggregateByMedic(ObjectId medic, AppointmentStatus... statuses) {
		Criteria conditions = Criteria.where("medic").is(medic);
		if (statuses.length > 0) {
			conditions = conditions.and("status").in(Arrays.asList(statuses));
		}
		AggregationOperation stage1 = TypedAggregation.match(conditions);
	...
	public Mono<List<LocalDateTime>> findBookedAppointmentDates(ObjectId clinic, ObjectId medic, ObjectId excludedId) {
		Criteria criteria = Criteria.where("medic").is(medic);
		if (excludedId != null) {
			criteria = criteria.and("_id").ne(excludedId);
		}
		AggregationOperation stage1 = TypedAggregation.match(criteria);
		AggregationOperation stage2 = TypedAggregation.match(Criteria.where("clinic").is(clinic));
		AggregationOperation stage3 = TypedAggregation.match(Criteria.where("status").is(AppointmentStatus.SCHEDULED));
	...
	public Flux<Appointment> aggregateByUser(ObjectId user, AppointmentStatus... statuses) {
		Criteria conditions = Criteria.where("user").is(user);
		if (statuses.length > 0) {
			conditions = conditions.and("status").in(Arrays.asList(statuses));
		}
		AggregationOperation stage1 = TypedAggregation.match(conditions);
	...
	public Mono<Appointment> aggregateById(ObjectId id) {
		...
		return mongodb.aggregate(aggregation, Appointment.KEY_SPACE, Appointment.class).next();
	}
