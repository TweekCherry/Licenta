<template>
  <v-dialog v-model="visible" scrollable width="400" :fullscreen="fullscreen">
    <v-card :loading="loading">
      ...
      <v-card-actions>
        <v-btn text block type="submit" color="green" @click="download">
...
export default {
  ...
  methods: {
    download() {
      this.loading = true
      backend.$downloadConsultation(this.consultationData.id).then(r => {
        this.downloadFile('consultation.pdf', new Blob([r.data], { type: 'application/octet-stream' }))
        this.showSuccessNotification('File successfully downloaded!')
      }).catch(e => this.showErrorNotification('An error occured while exporting the file'))
