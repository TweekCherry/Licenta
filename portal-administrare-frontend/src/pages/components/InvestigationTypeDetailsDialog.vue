<template>
  <v-dialog v-model="visible" :fullscreen="fullscreen" scrollable width="800">
    <ValidationObserver ref="observer" v-slot="{ handleSubmit }">
      <v-form @submit.prevent="handleSubmit(saveInvestigationType)">
        <v-card :loading="loading">
          <v-card-title class="justify-space-between">
            <div>
              <div>Investigation type details</div>
            </div>
            <div>
            ...
            <div class="row">
              <div class="col-12">
                <ValidationProvider rules="required" name="Name" v-slot="{ errors, valid }">
                  <v-text-field v-model="investigationTypeData.name" label="Name" required dense :error-messages="errors" :success="valid" focus name="name"/>
                </ValidationProvider>
              </div>
            </div>
          </v-card-text>
...
export default {
  name: 'InvestigationTypeDetailsDialog',
  components: {
  ...
  computed: {
    ...
    id() {
      if (this.investigationType !== null && this.investigationTypeData.name !== this.investigationType.name) {
        return this.investigationType.name
      }
      return null
    }
  },
  data() {
    return {
      fullscreen: false,
      loading: false,
      investigationTypeData: this.newInvestigationType()
    }
  },
  methods: {
    saveInvestigationType() {
      this.loading = true
    ...
    isVisible: function(newValue, oldValue) {
      if (newValue) {
        ...
        if (this.investigationType !== null) {
          this.investigationTypeData = this.clone(this.investigationType)
        } else {
          this.investigationTypeData = this.newInvestigationType()
        }
      }
    }
  }
